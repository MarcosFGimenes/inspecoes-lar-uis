...
import type { CorrectiveAssigneeOption, CorrectiveOsItem } from "../_types";
...
export default function CorrectivesPlannedList() {
  ...
  const assigneeMap = useMemo(() => {
    const map = new Map<string, CorrectiveAssigneeOption>();
  ...
  const resolveAssignees = useCallback(
    (item: CorrectiveOsItem): string => {
      const ids = [
        item.assignees?.owner ?? item.owner ?? null,
        item.assignees?.maintainer1 ?? item.maintainer1 ?? null,
        item.assignees?.maintainer2 ?? item.maintainer2 ?? null,
      ...
      const uniqueIds = ids.filter((value, index, array) => array.indexOf(value) === index);

      const labels = uniqueIds.map(id => {
        const option = assigneeMap.get(id);
        if (!option) {
          return id;
        }

        const pieces = [option.matricula, option.nome].filter(part => Boolean(part && part.trim()));
        return pieces.join(" — ") || option.id;
      });
  ...
  const formatOsNumber = useCallback((item: CorrectiveOsItem): string => {
  ...
  const formatSeverity = useCallback((severity: number | null): string => {
    if (!severity) {
      return "-";
    }
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[20%]">NC relacionada</TableHead>
                  ...
                  <TableHead className="w-[14%]">Responsáveis</TableHead>
                  ...
                  <TableRow key={item.id}>
                    <TableCell>
                      <div className="space-y-1">
                        <p className="font-medium text-[var(--text)]">{formatOsNumber(item)}</p>
                        <p className="text-xs text-[var(--muted)]">
                          {item.machineTag ? `TAG ${item.machineTag}` : "Sem TAG"}
                        </p>
                        {item.machineName ? (
                          <p className="text-xs text-[var(--muted)]">{item.machineName}</p>
                        ) : null}
                      </div>
                    </TableCell>
                    <TableCell>
                      {item.ncId ? (
                        <div>
                          <p className="text-sm font-medium text-[var(--text)]">
                            {item.ncDescription ?? "NC sem descrição"}
                          </p>
                    ...
                    <TableCell>{formatSeverity(item.effectiveSeverity)}</TableCell>
                    <TableCell className="whitespace-pre-line text-sm text-[var(--text)]">
                      {resolveAssignees(item)}
                    </TableCell>
                    <TableCell>
                      {item.status ? (
